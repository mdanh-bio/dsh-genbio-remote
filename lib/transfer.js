...
const DEFAULT_RCLONE_REMOTE = Object.freeze({ HPC: "hpc", genbioh100: "genbioh100", genbio_mdanh: "genbio01" });
...
function resolveRcloneRemote(config, target) {
  const override = config?.rcloneRemote?.[target];
  if (typeof override === "string" && override.length > 0) return override;
  const fallback = DEFAULT_RCLONE_REMOTE[target];
  if (typeof fallback === "string" && fallback.length > 0) return fallback;
  throw new Error(`no rclone remote configured for target ${target}: add an SFTP remote to ~/.rclone.conf and map it via config.rcloneRemote.${target}`);
}
...
export { DEFAULT_RCLONE_REMOTE, RCLONE_TRANSFER_ARGS, SSH_TRANSPORT_FAIL_RE, resolveRcloneRemote, rcloneCopytoCommand, rcloneCopyfromCommand, sleepMs, runRemoteWithRetry };
