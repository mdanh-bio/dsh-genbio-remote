...
function validateEnvelope(state, requested) {
  ...
  if (envelope.target !== "HPC" || envelope.partition !== "gpus") throw new Error("project operations require an HPC/gpus envelope");
...
async function cancelOwnedJob({ state, project, operation, jobId, exec, userQuestions, runRemote }) {
...
async function fetchArtifacts({ manifest, requested, exec, userQuestions, shell, runRemote, config }) {
  ...
  await writeFile(receiptPath, JSON.stringify({ receipt_schema: "genbio-project-fetch/1", project: manifest.project, fetched_at: new Date().toISOString(), source_root: manifest.remoteRoot, rclone_remote: rcloneRemote, transfer: "rclone-copyfrom", files: transferred }, null, 2) + "\n", "utf8");
...
export {
  strictRemote, validateEnvelope, assertAggregateCapacity, allocationsOf, submissionsOf,
  uniqueJobNameFor, verifyPackage, localFiles, runLocal, parseChecksums, parseSubmitOutput,
  isTransportFailure, reconcileSubmission, startTrackedJob, askTransfer, submitJob,
