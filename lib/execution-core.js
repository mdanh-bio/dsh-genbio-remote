...
export async function probeNodeHeadroom({ exec, runRemote, cpus, gpus, node }) {
  ...
  const freeGpus = configuredGpus === null ? Number.NaN : allocatedGpus === null ? (baseState === "idle" ? configuredGpus : Number.NaN) : configuredGpus - allocatedGpus;
  if (![cores, allocCores, freeCpus, freeGpus].every(Number.isFinite)) throw new Error(`${node} pre-submit probe: missing or non-numeric CPU/GPU resources "${value}"; failing closed before sbatch`);
