...
export async function probeNodeHeadroom({ exec, runRemote, cpus, gpus, node }) {
  ...
  const gpuEvidenceComplete = allocatedGpus !== null || baseState === "idle";
  const freeGpus = allocatedGpus === null ? configuredGpus : configuredGpus - allocatedGpus;
  if (![cores, allocCores, freeCpus].every(Number.isFinite) || (gpuEvidenceComplete && !Number.isFinite(freeGpus))) throw new Error(`${node} pre-submit probe: missing or non-numeric CPU/GPU resources "${value}"; failing closed before sbatch`);
  ...
  if (gpuEvidenceComplete && freeGpus < gpusReq) throw new Error(`${node} pre-submit probe: insufficient GPU headroom (free ${freeGpus} < requested ${gpusReq}); failing closed before sbatch`);
  return { ok: true, note: `${node} state=${stateText} free_cpus=${freeCpus} free_gpus=${gpuEvidenceComplete ? freeGpus : "scheduler-authority"}` };
}
