...
export async function probeNodeHeadroom({ exec, runRemote, cpus, gpus, node }) {
  ...
  const body = `set -eu; line=$(scontrol show node ${safeNode} -o 2>/dev/null || true); printf 'NODE_PROBE=%s\n' "$line"`;
  ...
  const gpuCount = (tres) => { const match = /(?:^|,)gres\/gpu(?:\/[^=,]+)?=(\d+)/u.exec(tres ?? ""); return match ? Number(match[1]) : 0; };
  const configuredGpus = gpuCount(field("CfgTRES"));
  const allocatedGpus = gpuCount(field("AllocTRES"));
  const freeCpus = cores - allocCores;
  const freeGpus = configuredGpus - allocatedGpus;
  if (![cores, allocCores, configuredGpus, allocatedGpus, freeCpus, freeGpus].every(Number.isFinite)) throw new Error(`${node} pre-submit probe: non-numeric node resources "${value}"; failing closed before sbatch`);
