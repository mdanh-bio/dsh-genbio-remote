...
export function shellQuote(value) { const text = String(value); if (!/^[A-Za-z0-9_./:=+@,-]+$/u.test(text)) throw new Error(`unsafe fixed path/token: ${text}`); return `'${text}'`; }
export function freshRunDirectoryCommand(remoteBase, remoteRunDir) {
  const parent = `${remoteBase}/runs`;
  return strictRemote("HPC", `set -eu; mkdir -p -m 700 -- ${shellQuote(parent)}; mkdir -m 700 -- ${shellQuote(remoteRunDir)}`);
}
