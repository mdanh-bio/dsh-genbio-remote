...
import { loadProject, manifestNames } from "./project-tools.js";
import { aggregateProjects } from "./aggregate.js";

function projectsDirOf(config) {
  return typeof config?.pinnedProjectsDir === "string" && config.pinnedProjectsDir.length > 0
    ? config.pinnedProjectsDir
    : "/Users/mdanh/.dsh/profiles/desktop/genbio-pinned-projects";
}

export function createProjectStatusTools({ makeTool, requirePolicy, requireState, publicState, config }) {
    ...
    async (_args, exec) => {
      ...
      for (const project of await manifestNames(projectsDir)) {
        try {
          ...
          projectSummaries.push({ project, valid: false, error: String(error?.message ?? error).slice(0, 300) });
        }
      }
