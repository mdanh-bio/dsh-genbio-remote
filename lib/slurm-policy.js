// Local-only, fail-closed validation for pinned Slurm batch templates.
...
// the real AI.zymes stage2 template).
...
function parseDirective(line, lineNumber, options) {
  ...
  if (!DIRECTIVE_NAMES.has(name)) throw new Error(`SBATCH policy: --${name} is not allowlisted for pinned jobs (line ${lineNumber})`);
...
export function validatePinnedSbatch(text, jobSpec, context = {}) {
  if (!jobSpec || !Number.isInteger(jobSpec.cpus) || !Number.isInteger(jobSpec.gpus) || !Number.isInteger(jobSpec.concurrency)) throw new Error("SBATCH policy: invalid pinned job resource specification");
  ...
  if (nodes !== 1) throw new Error("SBATCH policy: pinned submission requires exactly one node");
