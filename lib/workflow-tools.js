...
export function createWorkflowTools({ makeTool, requirePolicy, requireState, publicState, config, projectSource: suppliedProjectSource, projectPlanTool, projectExecuteTool, projectStatusTool, pinnedCancelTool, workflowRegistry }) {
  const projectSource = suppliedProjectSource ?? createProjectSource(config);

  async function resolveWorkflowPlan(name, exec, state, { completed = [] } = {}) {
    ...
    if (workflow.schema === "genbio-workflow/1") {
      const plan = await planWorkflow(workflow, { resolveProject: (project) => projectSource.loadProject(project, exec).then((item) => item.manifest), completed });
    ...
    const plan = await planWorkflow(workflow, { resolveOperation: async ({ project, operation, parameters, nodeId }) => {
      ...
      const built = buildOperationPlan({ project, operation, policyHash: state.policy.hash, manifestSha: loaded.manifestSha, resolution });
  ...
  const planTool = makeTool("genbio_workflow_plan", "Plan a local workflow. Schema-v1 is informational. Schema-v2 resolves every node into immutable policy-validated operation plans and stores one session-owned workflow plan. No transfer, allocation, or submission occurs.", { workflow: { type: "string", required: true }, completed: { type: "array", items: { type: "string" } } }, async (args, exec) => {
    const state = requireState(exec);
    const resolved = await resolveWorkflowPlan(String(args.workflow), exec, state, { completed: Array.isArray(args.completed) ? args.completed.map(String) : [] });
    if (resolved.workflow.schema === "genbio-workflow/2") {
    ...
    return { ok: true, status: { ...publicState(state), workflow_plan: clone(resolved.plan), workflow_origin: resolved.loadedWorkflow.origin.kind } };
  });
  ...
  async function startOrAdvance(planRecord, exec, state, existing = null) {
    ...
    let record = existing;
    if (!record) {
      record = await workflowRegistry.record(sessionId, { workflow: planRecord.plan.workflow, workflowPlanHash: planRecord.planHash, nodes: planRecord.plan.nodes.map((node) => ({ nodeId: node.id, project: node.project, operation: node.operation, operationPlanHash: node.operation_plan_hash, status: node.status === "ready" ? "ready" : "blocked" })) });
    }
    ...
    await workflowRegistry.updateNode(sessionId, record.workflowRunId, ready.nodeId, { status: run.workloadStatus === "unknown" ? "reconciling" : "submitted", slurmJobId: run.slurmJobId ?? null });
  ...
  const statusTool = makeTool("genbio_workflow_status", "Show and reconcile one durable workflow run. It may collect bounded scheduler evidence for its one active owned node, but never submits the next node.", { workflow_run_id: { type: "string", required: true } }, async (args, exec) => {
    ...
    const planRecord = plansOf(state).find((item) => item.planHash === record.workflowPlanHash);
    if (!planRecord) throw new Error("workflow plan expired from this session; refusing to infer dependencies");
    for (const node of record.nodes.filter((item) => ACTIVE_NODE.has(item.status) && item.slurmJobId)) {
      if (!projectStatusTool) break;
      await projectStatusTool.execute({ project: node.project, operation: node.operation, job_id: node.slurmJobId }, exec);
      const run = [...(state.runs ?? [])].reverse().find((item) => item.slurmJobId === node.slurmJobId && String(item.operation).startsWith(`pinned-${node.project}-`));
      if (!run) continue;
      const next = run.workloadStatus === "completed" ? "completed" : run.workloadStatus === "failed" ? "failed" : ["pending", "running"].includes(run.workloadStatus) ? run.workloadStatus : "reconciling";
      if (next !== node.status) await workflowRegistry.updateNode(sessionId, record.workflowRunId, node.nodeId, { status: next, slurmState: run.slurmStatus ?? null, exitCode: run.slurmExitCode ?? null, evidence: run.workloadEvidence ?? null });
    }
  ...
  const advanceTool = makeTool("genbio_workflow_advance", "Advance one durable workflow by submitting at most one currently ready node. Status reconciliation and completion evidence must make the node ready first.", { workflow_run_id: { type: "string", required: true } }, async (args, exec) => {
    ...
    const planRecord = plansOf(state).find((item) => item.planHash === existing.workflowPlanHash);
    if (!planRecord) throw new Error("workflow plan expired from this session");
  ...
  const cancelTool = makeTool("genbio_workflow_cancel", "Cancel exactly one active session-owned workflow node through the existing ownership-checked cancellation tool.", { workflow_run_id: { type: "string", required: true }, node_id: { type: "string", required: true } }, async (args, exec) => {
    ...
    if (!pinnedCancelTool) throw new Error("workflow cancellation integration is unavailable");
    const result = await pinnedCancelTool.execute({ project: node.project, job_id: node.slurmJobId }, exec);
