...
export function createProjectTools({ makeTool, requirePolicy, requireState, publicState, config, runRemote, shell, userQuestions, jobs, requireRemoteAccess, projectSource: suppliedProjectSource, executionRegistry = null }) {
      ...
      const run = startTrackedJob({
        ...
        runBody: async ({ exec: runExec }) => {
          ...
          try {
            ...
            if (durable) {
              const created = await runRemote("HPC", freshRunDirectoryCommand(durable.remoteBase, durable.remoteRunDir), runExec, 30000);
  ...
  const statusTool = makeTool(
    ...
    { project: { type: "string", required: true }, run_id: { type: "string" }, operation: { type: "string" }, job_id: { type: "string" } },
    async (args, exec) => {
      requirePolicy();
      const state = requireState(exec);
      ...
      if (args.run_id !== undefined) {
        ...
        if (!ownedDurable.slurmJobId && ownedDurable.sbatchIssued && ownedDurable.uniqueJobName !== "pending") {
          ...
          if (reconciledJobId) ownedDurable = await executionRegistry.update(ownedDurable.runId, (item) => ({ ...item, slurmJobId: reconciledJobId, workloadStatus: "submitted", allocationStatus: "nonterminal", workloadEvidence: "scheduler-reconciled-by-unique-name", note: null }));
        }
        if (ownedDurable.slurmJobId) {
      ...
      return { ok: true, status: { ...publicState(state), project, origin: loaded.origin.kind, plans, project_runs: clone(runs), durable_runs: durableRuns } };
    },
  );
