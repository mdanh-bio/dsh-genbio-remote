...
// Status vocabulary is shared with the session run records (index.js launch /
// pinned / aizyme) and the Phase 2 durable run registry (in-flight /
// reconciling). A non-terminal run means the operation lane is active.
...
/** Does a session run record belong to `project`? Mirrors the existing
 *  genbio_project_status correlation: `project-<name>` (one-shot project lane)
 *  and `pinned-<name>-<op>` (pinned jobs). The AI.zymes lane uses the implicit
 *  project id "aizyme" with `aizyme-<stage>` operations. */
export function runBelongsToProject(run, project) {
  ...
  if (project === "aizyme") return operation === "aizyme" || operation.startsWith("aizyme-");
  return operation === `project-${project}` || operation.startsWith(`pinned-${project}-`);
}

/** Map a run record to its MANIFEST operation name for this project, or null
 *  when the run does not name one of the project's declarative operations
 *  (e.g. the `project-<name>` one-shot lane carries no operation). The pinned
 *  lane names runs `pinned-<project>-<operation>`; the AI.zymes lane names
 *  them `aizyme-<stage>`. */
export function projectOperationOfRun(run, project) {
  ...
  const pinnedPrefix = `pinned-${project}-`;
  if (operation.startsWith(pinnedPrefix)) return operation.slice(pinnedPrefix.length) || null;
  if (operation === `project-${project}`) return null;
  if (project === "aizyme") {
    if (operation === "aizyme") return null;
    if (operation.startsWith("aizyme-")) return operation.slice("aizyme-".length) || null;
  }
  return null;
}
