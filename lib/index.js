...
import { z } from "zod";
import { createAizymeTools } from "./aizyme.js";
import { createPinnedTools } from "./pinned.js";
...
/** Phase 2 durable run registry root (config.runRegistryDir wins). Local only. */
const DEFAULT_RUN_REGISTRY_DIR = "/Users/mdanh/.dsh/profiles/desktop/genbio-run-registry";
...
function parseRunEvidence(run) {
  const stdout = String(run.stdout || "");
  const schedulerJobId = stdout.match(/^JOB_ID=([^\s]+)$/m)?.[1] ?? null;
  const remoteRunDir = stdout.match(/^RUN_DIR=(.+)$/m)?.[1]?.trim() ?? null;
  const schedulerRow = stdout.split(/\r?\n/).find(line => schedulerJobId && line.startsWith(`${schedulerJobId}|`));
  const fields = schedulerRow?.split("|") ?? [];
  return {
    schedulerJobId,
    remoteRunDir,
    terminalEvidence: schedulerRow ? { state: fields[1] || null, exitCode: fields[2] || null } : null,
...
const PROJECT_STATUS_PROJECTION_FIELDS = [
  "project", "valid", "schema_version", "error", "operations", "plans", "plan_count",
...
function projectionInit() {
  return { calls: {}, envelope: null, runs: [], projects: [], projects_status: [] };
}
...
function projectionView(state) {
  return { envelope: state.envelope, runs: state.runs, projects: state.projects, projects_status: state.projects_status };
}
...
async function apply(ctx, config = {}) {
  ...
  // Phase 2 durable bounded local run registry (local-only): per-session
  // records with atomic writes, restart load, and in-flight -> reconciling
  // reconciliation. Never stores secrets or raw logs. A registry failure is
  // surfaced by the adapters (run.registryError) and never alters compute.
  const runRegistry = createRunRegistry(typeof config.runRegistryDir === "string" && config.runRegistryDir.length > 0 ? config.runRegistryDir : DEFAULT_RUN_REGISTRY_DIR);
  ...
  const { inspect: aizymeInspect, stageTool: aizymeStage } = createAizymeTools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess, runRegistry });

  const { stageTool: pinnedStage, jobTool: pinnedJob, statusTool: pinnedStatus, cancelTool: pinnedCancel, fetchTool: pinnedFetch } = createPinnedTools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess });
  const { projectsTool, describeTool: projectDescribe, inventoryTool, planTool: projectPlan, executeTool: projectExecute, statusTool: projectStatus } = createProjectTools({ makeTool, requirePolicy, requireState, publicState, config, runRemote, shell, userQuestions, jobs, requireRemoteAccess });
  const { planTool: workflowPlanTool } = createWorkflowTools({ makeTool, requirePolicy, requireState, publicState, config });
  const { statusAllTool: projectsStatus } = createProjectStatusTools({ makeTool, requirePolicy, requireState, publicState, config });

  const disposePrompt = ctx.systemPrompt.section({
    name: "genbio-remote:policy",
    order: 104,
    text: "For Genbio remote work, use only the genbio_* tools. Never use generic bash, raw ssh, raw scp, alternate aliases, or ad hoc scheduler commands. All plugin-managed data transfers use rclone over SFTP (lib/transfer.js); scp is never invoked for data movement. First call genbio_policy_status, then genbio_preflight, then genbio_set_envelope before any launch or allocation. Respect separate policies for HPC, genbio_mdanh, and genbioh100. HPC node selection is manifest-driven: each project YAML declares its allowed node(s), validated against the policy allowlist at plan time; max 2 GPUs concurrent across all operations. genbioh100 is GPU 0 only; GPU 1 and gpu_util are protected; the current cap is 16 CPU threads, 32 GB, and one GPU workload. Use genbio_validate_resources before each workload. If resources exceed the envelope, wait for the interactive resource-selection question; custom guidance alone is not approval. Use genbio_monitor and genbio_runs for owned job state, elapsed time, and bounded logs/errors. Remote folder access is granted per workspace: each operation may only touch remote paths under the session workspace's granted remote folders (rw covers reads and writes, ro reads only) or under the policy's read-only tool roots (conda, module tooling); genbio_runs status lists the session's on-the-fly remoteGrants. If an operation needs an ungranted remote folder, the plugin pauses with a user question — wait for that answer; never route around it with alternate paths or raw commands. Use genbio_pinned_fetch to retrieve manifest-allowlisted small report/artifact files from a pinned project's fixed HPC run directory (byte-capped, SHA-256 matched on both sides); it is read-only remotely and needs no allocation or envelope. After terminal evidence is verified, separate observations from interpretation and call genbio_finalize_run with only durable findings and curated artifact references. Use genbio_publish_run only to retry the frozen record, and report memory publication separately from compute success. Use genbio_project_inventory for the content-addressed package digest of a project's manifest file set, and genbio_workflow_plan to mark which workflow DAG nodes are ready given a completed set; both are local-only read-only planning tools and never transfer, allocate, or submit. Use genbio_project_plan to resolve a schema-v2 declarative recipe into an immutable, policy-validated SBATCH wrapper plus a deterministic session plan hash, then genbio_project_execute to run that approved plan: it FRESH re-reads the manifest and the active policy, re-resolves the recipe with the plan's stored parameters, and requires the recomputed plan hash to equal the approved one (drift rejection before any side effect); it stages the manifest package and the content-addressed recipe wrapper (rclone-only, SHA-256 verified, clean-env bash -n) and submits through the shared atomic exact-once pinned core (envelope fit, aggregate capacity, atomic pair-lock admission, one sbatch, read-only reconciliation) exactly as genbio_pinned_job, and an ambiguous submission is never resubmitted. Use genbio_projects_status for the read-only local aggregate of project status (operations with resources, session plans, run rollups, and which operations are active, suggested next, or need review); it performs no remote access, transfer, allocation, or scheduler query."
  });
  ...
  ctx.tools.register(aizymeInspect);
  ctx.tools.register(aizymeStage);
  ctx.tools.register(pinnedStage);
  ctx.tools.register(pinnedJob);
  ctx.tools.register(pinnedStatus);
  ctx.tools.register(pinnedCancel);
  ctx.tools.register(pinnedFetch);
