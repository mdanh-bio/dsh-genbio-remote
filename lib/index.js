...
import { createExecutionRegistry } from "./execution-registry.js";
import { createAizymeH100Tools } from "./aizyme-h100.js";
import { createH100DirectTools } from "./h100-direct.js";
import { createH100MirrorTools } from "./h100-mirror.js";
...
const DEFAULT_DSH_PROFILE_DIR = join(HOME_DIR, ".dsh/profiles/desktop");
const DEFAULT_WORKFLOW_REGISTRY_DIR = join(DEFAULT_DSH_PROFILE_DIR, "genbio-workflow-registry");
const DEFAULT_EXECUTION_REGISTRY_DIR = join(DEFAULT_DSH_PROFILE_DIR, "genbio-execution-registry");
const DEFAULT_RUN_REGISTRY_DIR = join(DEFAULT_DSH_PROFILE_DIR, "genbio-run-registry");
...
async function apply(ctx, config = {}) {
  const policyPath = config.policyPath ?? join(HOME_DIR, ".codex/skills/operate-genbio-hpc-remote/references/genbio-compute-policy.yaml");
  ...
  const executionRegistry = createExecutionRegistry(typeof config.executionRegistryDir === "string" && config.executionRegistryDir.length > 0 ? config.executionRegistryDir : DEFAULT_EXECUTION_REGISTRY_DIR);
  ...
  await readPolicy();
  const disposePoll = timer.interval(() => { void readPolicy(); }, Number(config.policyPollMs ?? 15000));

  const setError = (state, error) => { state.lastError = String(error?.message ?? error); };
  const rehydrateExecutionState = async (id, state) => {
    if (state.executionRegistryHydrated) return;
    if (state.runs.length > 0 || state.allocations.length > 0 || state.submissions.length > 0) {
      state.executionRegistryHydrated = true;
      return;
    }
    const durableRuns = (await executionRegistry.list()).filter((record) => record.sessionId === id);
    for (const record of durableRuns.slice(-50)) {
      const run = {
        runId: record.runId,
        attemptId: record.attemptId,
        target: record.target,
        operation: `project-${record.project}-${record.operation}`,
        status: record.status,
        helperStatus: record.helperStatus,
        ...
        slurmStatus: record.slurmState,
        slurmExitCode: record.exitCode,
        slurmElapsed: record.elapsed,
        workloadEvidence: record.workloadEvidence,
        resources: { cpus: record.cpus, gpus: record.gpus, memGb: null, concurrency: record.concurrency },
        policyHash: record.policyHash,
        policyHashMatches: state.policy?.hash === record.policyHash,
        node: record.node,
        partition: record.partition,
        remoteRunDir: record.remoteRunDir,
        envelope: null,
        memory: { status: "not-finalized", error: null, openVikingSessionId: null, traceId: null },
      };
      if (!state.runs.some((item) => item.runId === run.runId)) state.runs.push(run);
      if (record.slurmJobId && !state.submissions.some((item) => item.slurmJobId === record.slurmJobId)) state.submissions.push({ project: record.project, operation: record.operation, uniqueJobName: record.uniqueJobName, slurmJobId: record.slurmJobId, status: "submitted", token: record.token, durableRunId: record.runId });
      if (["submitting", "nonterminal", "ambiguous", "cancel-requested"].includes(record.allocationStatus) && !state.allocations.some((item) => item.durableRunId === record.runId)) state.allocations.push({ project: record.project, operation: record.operation, slurmJobId: record.slurmJobId, status: record.allocationStatus, cpus: record.cpus, gpus: record.gpus, concurrency: record.concurrency, durableRunId: record.runId, source: "durable-rehydration" });
    }
    state.executionRegistryHydrated = true;
  };
  const stateFor = (id) => {
    ...
    if (!state) {
      state = {
        ...
        allocations: [],
        submissions: [],
        executionRegistryHydrated: false,
        lastError: null,
        remoteGrants: [],
      };
      states.set(id, state);
  ...
  const hydrateStateFor = async (exec) => {
    const id = sessionIdOf(exec);
    const state = stateFor(id);
    try {
      await rehydrateExecutionState(id, state);
    } catch (error) {
      setError(state, error);
      throw new Error(`durable Genbio execution state could not be loaded: ${String(error?.message ?? error)}`);
    }
    return state;
  };
  const requirePolicy = () => {
  ...
  const setEnvelope = makeTool(
    ...
    async (args, exec) => {
      ...
      if (args.target === "genbioh100") {
        ...
        if (args.max_gpus > 1 || args.max_cpus > limits.cpu_threads_per_job || args.mem_gb !== undefined && args.mem_gb > limits.mem_gb_per_job || args.concurrency > classCap || args.node !== "genbioh100") throw new Error("genbioh100 envelope exceeds GPU0-only, CPU, memory, or class concurrency policy");
      }
  ...
  const finalizeRun = makeTool(
    ...
    async (args, exec) => {
      ...
      if (!TERMINAL_RUN_STATES.has(run.helperStatus ?? run.status)) throw new Error(`Genbio helper must reach a terminal state (completed/failed/killed) before finalization; current helperStatus=${run.helperStatus ?? "unset"}, status=${run.status ?? "unset"}`);
  ...
  const runs = makeTool(
    ...
    {},
    async (_args, exec) => ({ ok: true, status: publicState(await hydrateStateFor(exec)) }),
  );

  const { stageTool: aizymeH100Stage, statusTool: aizymeH100Status } = createAizymeH100Tools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess, runRegistry });
  const { stageTool: h100DirectStage, jobTool: h100DirectJob, statusTool: h100DirectStatus, fetchTool: h100DirectFetch } = createH100DirectTools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess, runRegistry });
  const { planTool: h100MirrorPlan, executeTool: h100MirrorExecute, statusTool: h100MirrorStatus } = createH100MirrorTools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, config, requireRemoteAccess });
  const projectSource = createProjectSource(config);
  const { projectsTool, describeTool: projectDescribe, inventoryTool, planTool: projectPlan, executeTool: projectExecute, statusTool: projectStatus, cancelTool: projectCancel, fetchTool: projectFetch } = createProjectTools({ makeTool, requirePolicy, requireState, publicState, config, runRemote, shell, userQuestions, jobs, requireRemoteAccess, projectSource, executionRegistry, logger: ctx.logger, hydrateState: hydrateStateFor });
  ...
  ctx.tools.register(aizymeH100Stage);
  ctx.tools.register(aizymeH100Status);
  ctx.tools.register(h100DirectStage);
  ctx.tools.register(h100DirectJob);
  ctx.tools.register(h100DirectStatus);
  ctx.tools.register(h100DirectFetch);
  ctx.tools.register(h100MirrorPlan);
  ctx.tools.register(h100MirrorExecute);
  ctx.tools.register(h100MirrorStatus);
