...
// Schema v1 remains the Phase-2 compatibility surface. Schema v2 adds bounded
// plain JSON parameters and content-addressed operation resolutions. Planning is
...
/** Strict fail-closed parse of a schema-v1 or schema-v2 workflow definition. */
export function parseWorkflow(definition) {
  ...
  if (version !== 1 && version !== 2) throw new Error("workflow schema_version must be 1 or 2");
  ...
  for (const [index, node] of definition.nodes.entries()) {
    ...
    assertKeys(node, new Set(version === 2 ? ["id", "project", "operation", "depends_on", "parameters"] : ["id", "project", "operation", "depends_on"]), `workflow node ${index}`);
    ...
    if (version === 2) parsed.parameters = node.parameters === undefined ? Object.freeze({}) : normalizeParameters(node.parameters, `workflow node ${node.id} parameters`);
  ...
  assertAcyclic(nodes);
  return Object.freeze({ schema: `genbio-workflow/${version}`, name: definition.workflow, nodes: Object.freeze(nodes), pairs: Object.freeze(Object.fromEntries(pairs)) });
}
...
/** Plan one parsed workflow. V1 output remains byte-shape compatible. */
export async function planWorkflow(workflow, { resolveProject, resolveOperation, completed = [] } = {}) {
  if (!workflow || !["genbio-workflow/1", "genbio-workflow/2"].includes(workflow.schema) || !Array.isArray(workflow.nodes)) throw new Error("planWorkflow requires a parsed workflow");
  const version = workflow.schema.endsWith("/2") ? 2 : 1;
  if (typeof resolveProject !== "function" && typeof resolveOperation !== "function") throw new Error("planWorkflow requires a manifest reader (resolveProject) or operation resolver (resolveOperation)");
  if (version === 2 && typeof resolveOperation !== "function") throw new Error("schema-v2 workflow planning requires resolveOperation");
  ...
  for (const node of workflow.nodes) {
    ...
    if (typeof resolveOperation === "function") {
      try {
        resolution = normalizeResolution(await resolveOperation({ project: node.project, operation: node.operation, parameters: node.parameters ?? Object.freeze({}), nodeId: node.id }), node);
      } catch (error) {
        throw new Error(`workflow node ${node.id}: operation resolution failed: ${String(error?.message ?? error)}`);
      }
    } else {
      let manifest;
      try { manifest = await resolveProject(node.project); }
      catch (error) { throw new Error(`workflow node ${node.id}: project ${node.project} unavailable: ${String(error?.message ?? error)}`); }
    ...
    const item = { id: node.id, project: node.project, operation: node.operation, pairKey: node.pairKey, dependsOn: node.dependsOn, status, waitingOn: Object.freeze(waitingOn) };
    if (version === 2) Object.assign(item, { parameters: node.parameters, operation_plan_hash: resolution.planHash, resources: resolution.resources, origin: resolution.origin });
    planned.push(Object.freeze(item));
  }
  const byStatus = (status) => Object.freeze(planned.filter((node) => node.status === status).map((node) => node.id));
  const result = { schema: `genbio-workflow-plan/${version}`, workflow: workflow.name, nodes: Object.freeze(planned), ready: byStatus("ready"), blocked: byStatus("blocked"), completed: byStatus("completed") };
  if (version === 2) {
    ...
    result.workflow_plan_hash = workflowPlanHashOf(immutable);
  }
