...
export function createExecutionRegistry(rootDir) {
  ...
  async function update(runId, mutate) {
    return transact((records) => {
      ...
      if (candidate.runId !== before.runId || candidate.attemptId !== before.attemptId || candidate.token !== before.token || candidate.planHash !== before.planHash) throw new Error("execution registry immutable identity changed");
  ...
  async function reserve({ sessionId, workspace, project, operation, planHash, manifestSha, packageSha, wrapperSha, policyHash, remoteBase, cpus, gpus, concurrency, node, partition, envelope }) {
    return transact((records) => {
      ...
      if (existing) throw new Error(`${project}: operation ${operation} has durable active attempt ${existing.runId} (${existing.allocationStatus}); reconcile it before another submission`);
