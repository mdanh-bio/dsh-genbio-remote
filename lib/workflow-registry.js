...
function normalizeNode(node) {
  ...
  assertKeys(node, new Set(["nodeId", "project", "operation", "status", "operationPlanHash", "slurmJobId", "slurmState", "exitCode", "evidence", "createdAt", "updatedAt", "finishedAt"]), "workflow registry node");
...
function normalizeRecord(record) {
  ...
  return { workflowRunId: record.workflowRunId, workflow: record.workflow, workflowPlanHash: record.workflowPlanHash, status: record.status, nodes, createdAt: record.createdAt, updatedAt: record.updatedAt, finishedAt: record.finishedAt ?? null };
}
...
export function createWorkflowRegistry(rootDir) {
  ...
  function entryOf(sessionId) {
    ...
    if (!entry) {
      ...
      entry = { dir, file: join(dir, "workflows.json"), records: null, chain: Promise.resolve() };
  ...
  async function persist(entry) {
    ...
    if (Buffer.byteLength(payload, "utf8") > MAX_FILE_BYTES) throw new Error(`workflow registry exceeds ${MAX_FILE_BYTES} serialized bytes; refusing to write`);
  ...
  async function record(sessionId, input) {
    ...
    return queue(entry, async () => {
      ...
      assertKeys(input, new Set(["workflowRunId", "workflow", "workflowPlanHash", "status", "nodes", "createdAt"]), "workflow registry input");
      ...
      const candidate = normalizeRecord({
        ...
        nodes: input.nodes.map((node) => {
          ...
          assertKeys(node, new Set(["nodeId", "project", "operation", "operationPlanHash", "status", "slurmJobId", "slurmState", "exitCode", "evidence"]), "workflow registry input node");
  ...
  async function updateNode(sessionId, workflowRunId, nodeId, patch) {
    ...
    return queue(entry, async () => {
      ...
      assertKeys(patch, new Set(["status", "slurmJobId", "slurmState", "exitCode", "evidence"]), "workflow registry node patch");
      ...
      for (const field of ["slurmJobId", "slurmState", "exitCode", "evidence"]) if (patch[field] !== undefined) node[field] = patch[field];
      node.updatedAt = now;
