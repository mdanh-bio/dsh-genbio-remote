...
// ── Generic pinned-project Slurm operations ────────────────────────────────────
// One user-owned YAML manifest per project (in the pinned-projects directory)
// describes: local file set (staged verbatim into a fixed remote run dir), and
// the Slurm templates to submit with their pinned CPU/GPU counts and
// concurrency. All ENFORCEMENT
// stays in code — envelope fit, folder grants, material-transfer approval,
// rclone-only data transfer (never scp; see lib/transfer.js), strict native
// OpenSSH for remote commands, checksum verification, manifest-driven node selection, fail-closed checks.
// Manifests are re-read on every tool call (lazy load): adding a new project is
// one YAML file, no code change, no bundle sync, no app restart.
...
/** Strict manifest validation — anything malformed throws (fail closed). */
function parseManifest(project, parsed) {
  ...
  if (m.python_bin !== undefined && (typeof m.python_bin !== "string" || !m.python_bin.startsWith("/") || m.python_bin.includes(".."))) throw new Error(`${project}: python_bin must be an absolute path without ..`);
  ...
  const extraDirs = m.extra_dirs ?? [];
  if (!Array.isArray(extraDirs) || extraDirs.length > MAX_EXTRA_DIRS) throw new Error(`${project}: extra_dirs must be an array of at most ${MAX_EXTRA_DIRS} relative paths`);
  for (const rel of extraDirs) assertRelativePath(rel);
  ...
  for (const [name, spec] of Object.entries(m.jobs)) {
    ...
    assertRelativePath(spec.template);
    const cpus = spec.cpus; const gpus = spec.gpus ?? 0; const concurrency = spec.concurrency ?? 1;
    if (typeof cpus !== "number" || !Number.isInteger(cpus) || cpus < 1 || cpus > 128) throw new Error(`${project}: job ${name} cpus must be an integer 1..128`);
    if (typeof gpus !== "number" || !Number.isInteger(gpus) || gpus < 0 || gpus > 8) throw new Error(`${project}: job ${name} gpus must be an integer 0..8`);
    if (typeof concurrency !== "number" || !Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) throw new Error(`${project}: job ${name} concurrency must be an integer 1..8`);
    jobSpecs[name] = Object.freeze({ template: spec.template, cpus, gpus, concurrency });
    count += 1;
  }
  ...
  return Object.freeze({ project: m.project, localRoot: m.local_root, remoteRoot: m.remote_root, pythonBin: m.python_bin ?? null, files: Object.freeze(m.files), extraDirs: Object.freeze(extraDirs), jobs: Object.freeze(jobSpecs), fetch: fetchSpec, description: typeof m.description === "string" ? m.description : "" });
}

// Load one manifest on demand (lazy, per tool call): the projects directory is
// re-read each time, so a newly dropped manifest is immediately available and
// a removed one immediately unavailable. Unknown/malformed → clear error that
// lists what is actually present.
async function loadManifest(projectsDir, project) {
  ...
  try { text = await readFile(path, "utf8"); } catch {
    let available = [];
    try { available = (await readdir(projectsDir)).filter((name) => name.endsWith(".yaml")).map((name) => name.replace(/\.yaml$/u, "")).sort(); } catch { /* dir unreadable: report below */ }
    throw new Error(`unknown pinned project: ${project} (available: ${available.join(", ") || "none"})`);
  }
  return parseManifest(project, parseYaml(text));
}
...
// Phase-0 gate: validate the exact local batch template before prompting for a
// remote folder grant or starting a background job. Returns the frozen
// validation plan plus the SHA-256 of the EXACT validated bytes (t7 / t5#4):
// the background submission path must prove the bytes it is about to submit
// hash to this same digest — a template mutated between validation and
// submission can never reach sbatch.
async function validateLocalJobTemplate(manifest, operation, policy, state) {
  const jobSpec = manifest.jobs[operation];
  if (!jobSpec.template.endsWith(".sbatch")) throw new Error(`${manifest.project}: job ${operation} template must end in .sbatch`);
  if (!manifest.files.includes(jobSpec.template)) throw new Error(`${manifest.project}: job ${operation} template must be included in files`);
  const templatePath = join(manifest.localRoot, jobSpec.template);
  const info = await stat(templatePath);
  if (!info.isFile()) throw new Error(`${manifest.project}: job template is not a regular file: ${jobSpec.template}`);
  const bytes = await readFile(templatePath);
  const plan = validatePinnedSbatch(bytes.toString("utf8"), jobSpec, { policy, envelope: state.envelope });
  return { plan, templateSha: sha256(bytes) };
}

// Pre-submit local digest re-check (t7 / t5#4): re-read the template and
// require it to hash to the Phase-0 validated digest before ANY remote effect.
// Closes the TOCTOU window between validation and submission: the bytes the
// remote sbatch would run must be the bytes the policy validator saw.
async function assertTemplateDigestUnchanged(manifest, operation, templateSha) {
  const templatePath = join(manifest.localRoot, manifest.jobs[operation].template);
  const bytes = await readFile(templatePath);
  if (sha256(bytes) !== templateSha) throw new Error(`${manifest.project}: template ${manifest.jobs[operation].template} changed after Phase-0 validation (digest mismatch); nothing is submitted — re-run genbio_pinned_job to validate and submit the current bytes`);
}

// Policy shared_checks.global.approval-gates: material-transfer-in requires a
// separate explicit owner approval (mirrors the DAE staging prompt).
...
function validateEnvelope(state, requested) {
  ...
  if (!envelope) throw new Error("set the HPC session envelope before running a pinned operation");
  if (envelope.target !== "HPC" || envelope.partition !== "gpus") throw new Error("pinned operations require an HPC/gpus envelope");
  ...
  if (requested.cpus > envelope.maxCpus || requested.gpus > envelope.maxGpus || requested.concurrency > envelope.concurrency) throw new Error(`pinned operation (${requested.cpus} CPUs) exceeds the current HPC envelope`);
}
...
// scheduler evidence (genbio_pinned_status marks the allocation terminal
...
function markAllocationTerminal(state, slurmJobId, stateText) {
  let found = false;
  for (const entry of allocationsOf(state)) if (entry.slurmJobId === slurmJobId && entry.status === "nonterminal") { entry.status = "terminal"; entry.terminalState = stateText ?? null; entry.terminalAt = Date.now(); found = true; }
  return found;
}
function assertAggregateCapacity(state, requested) {
  const envelope = state.envelope;
  const outstanding = allocationsOf(state).filter((entry) => entry.status === "submitting" || entry.status === "nonterminal");
...
// aizyme.js strictRemote: JSON.stringify emits a double-quoted shell argument,
...
// Stage the manifest's pinned files (owner-approved), build the remote
...
// additive and default-empty, so the pinned v1 staging path is byte-for-byte
// unchanged; the declarative recipe executor passes ["genbio-recipes"] so that
...
async function stageAndValidate({ manifest, exec, userQuestions, shell, runRemote, config, extraOwnPrefixes = [] }) {
  ...
  const segments = [`set -eu`, `cd -- ${root}`, `sha256sum ${rels} > PREPARED_SHA256.txt`];
  ...
  segments.push("printf 'PINNED_STAGE_VALIDATION_OK\\n'");
...
// (printf under set -eu): sinfo reports state|totalCores|allocCores|
// gresGpus|allocGresGpus for the target node; only idle/alloc/mix are usable,
// and free CPU/GPU must cover the requested resources. Any failure — nonzero
// exit, missing node, unusable state, unparseable row, or short headroom —
// throws BEFORE the submit call, so no allocation is issued.
//
// Stub accommodation (documented, production-unreachable): the real command
// ALWAYS emits the NODE_PROBE= marker (the printf runs unconditionally under
// set -eu, even when sinfo errors via `|| true` — then the value is empty and
// we fail closed on "no node"). Output WITHOUT the marker can only come
// from harness stubs that answer non-sbatch commands generically; there the
// exit code is the authority and the probe is a no-op.
const USABLE_NODE_STATES = new Set(["idle", "alloc", "mix"]);
export async function probeNodeHeadroom({ exec, runRemote, cpus, gpus, node }) {
  if (typeof node !== "string" || !node) throw new Error("probeNodeHeadroom requires a target node");
  ...
  const safeNode = node.replace(/'/gu, "");
  const body = `set -eu; line=$(sinfo -h -N -n ${safeNode} -o '%T|%c|%a|%G|%g' 2>/dev/null || true); printf 'NODE_PROBE=%s\\n' "$line"`;
  ...
  if (probeLine === undefined) return { ok: true, note: `${node} probe: no probe record in output (stub/no-op); exit code authority stood` };
  const value = probeLine.slice("NODE_PROBE=".length).trim();
  if (value === "") throw new Error(`${node} pre-submit probe: sinfo reported no such node; failing closed before sbatch`);
  const parts = value.split("|").map((p) => p.trim());
  if (parts.length < 5) throw new Error(`${node} pre-submit probe: unparseable sinfo row "${value}"; failing closed before sbatch`);
  const [stateText, cores, allocCores, gresGpus, allocGresGpus] = parts;
  const baseState = stateText.split("+")[0];
  ...
  const freeCpus = Number(cores) - Number(allocCores);
  const freeGpus = Number(gresGpus) - Number(allocGresGpus);
  if (!Number.isFinite(freeCpus) || !Number.isFinite(freeGpus)) throw new Error(`${node} pre-submit probe: non-numeric node resources "${value}"; failing closed before sbatch`);
  ...
  if (freeGpus < gpusReq) throw new Error(`${node} pre-submit probe: insufficient GPU headroom (free ${freeGpus} < requested ${gpusReq}); failing closed before sbatch`);
  return { ok: true, note: `${node} state=${stateText} free_cpus=${freeCpus} free_gpus=${freeGpus}` };
}
...
async function stageRecipeWrapper({ manifest, wrapperBytes, operation, manifestSha, exec, userQuestions, shell, runRemote, config }) {
  ...
  try {
    ...
    // env, no profile/rc — the same fixed binary the pinned stage uses.
...
async function reconcileSubmission({ exec, runRemote, jobName }) {
  const body = `set -eu; squeue_table=$(squeue -h -n '${jobName}' -o '%.18i|%.100j|%.20T' 2>/dev/null || true); sacct_table=$(sacct -X -n --name='${jobName}' --starttime=-1h --format=JobIDRaw,JobName,State -P 2>/dev/null || true); printf 'SQUEUE_BEGIN\\n%s\\nSQUEUE_END\\nSACCT_BEGIN\\n%s\\nSACCT_END\\n' "$squeue_table" "$sacct_table"`;
  const result = await runRemote("HPC", strictRemote("HPC", body), exec, 30000);
  if (result.exitCode !== 0) return null;
  return resolveReconciliationCandidates(parseReconciliationEnvelope(result.stdout, jobName));
}

// Submit exactly one pinned template from the remote run dir and return
...
// evidence is collected with genbio_pinned_status.
//
...
// Unique job name per submission intent (t10): the pinned job name gets a
...
function uniqueJobNameFor(jobName, token) {
  ...
  if (jobName.length + suffix.length > 100) return jobName;
...
async function submitJob({ manifest, operation, policy, state, exec, runRemote, intent, allocation, templateSha, templateRel, templateBytes }) {
  const project = manifest.project;
  const jobSpec = manifest.jobs[operation];
  ...
  // 1. TOCTOU digest binding + re-validation of the bytes about to be submitted.
  //    Two sources, both binding the exact bytes the policy validator saw:
  //      (a) templateBytes (a generated recipe wrapper held in memory): assert
  //          sha256(templateBytes) === templateSha and re-validate the in-memory
  //          text — the local manifest file is NEVER touched;
  //      (b) no templateBytes (the pinned v1 path): byte-identical to before —
  //          re-read the local template, require it to still hash to the
  //          Phase-0 validated digest, and re-validate the re-read text.
  let revalidated;
  if (templateBytes !== undefined) {
    ...
    revalidated = validatePinnedSbatch(templateBytes.toString("utf8"), jobSpec, { policy, envelope: state.envelope });
  } else {
    await assertTemplateDigestUnchanged(manifest, operation, templateSha);
    revalidated = validatePinnedSbatch((await readFile(join(manifest.localRoot, jobSpec.template), "utf8")), jobSpec, { policy, envelope: state.envelope });
  }
  ...
  // the manifest template (jobSpec.template) for the pinned v1 path. It is
  // relative to the run-dir root (the sbatch cwd), so the wrapper's
  // cd "$SLURM_SUBMIT_DIR" still lands in the manifest run dir.
  const submitTemplate = templateRel ?? jobSpec.template;
  ...
  if (submit.exitCode !== 0) {
    if (isTransportFailure(submit)) {
      ...
      throw new Error(`${project}: sbatch transport failure is ambiguous (token ${intent.token}, job name ${intent.uniqueJobName}); the job may have been accepted. Nothing is resubmitted; reconcile read-only (genbio_pinned_status / cluster inspection) before retrying`);
    }
...
function findWorkloadRun(state, project, jobId) {
  return [...(state.runs ?? [])].reverse().find((run) => run.target === "HPC" && run.slurmJobId === jobId && String(run.operation).startsWith(`pinned-${project}-`) && !String(run.operation).includes("status-")) ?? null;
}

function findOwnedOperationRun(state, project, operation, jobId) {
  return [...(state.runs ?? [])].reverse().find((run) => run.target === "HPC" && run.slurmJobId === jobId && run.operation === `pinned-${project}-${operation}`) ?? null;
}
...
function updateWorkloadFromScheduler(run, scheduler) {
  ...
  run.workloadEvidence = scheduler.workloadOutputSeen ? "scheduler-and-job-output" : "scheduler-only";
  if (TERMINAL_SCHEDULER_STATES.has(scheduler.state)) {
    run.workloadStatus = scheduler.state === "COMPLETED" && scheduler.exitCode === "0:0" && scheduler.workloadOutputSeen ? "completed" : "failed";
    run.workloadFinishedAt ??= Date.now();
  } else if (PENDING_SCHEDULER_STATES.has(scheduler.state)) run.workloadStatus = "pending";
  else if (RUNNING_SCHEDULER_STATES.has(scheduler.state)) run.workloadStatus = "running";
  else run.workloadStatus = "unknown";
  return true;
}
...
async function statusJob({ manifest, jobId, state, exec, runRemote }) {
  ...
  const root = shellQuote(manifest.remoteRoot);
  const body = `set -eu; run_dir=${root}; job=${shellQuote(jobId)}; table=$(sacct -X -j "$job" --format=JobIDRaw,JobName,State,ExitCode,Elapsed -P || true); printf 'SACCT=%s\\n' "$table"; line=$(printf '%s\\n' "$table" | awk -F'|' 'NR==2'); job_name=$(printf '%s' "$line" | awk -F'|' '{print $2}'); state=$(printf '%s' "$line" | awk -F'|' '{print $3}'); exit_code=$(printf '%s' "$line" | awk -F'|' '{print $4}'); elapsed=$(printf '%s' "$line" | awk -F'|' '{print $5}'); if test -n "$job_name"; then for suffix in out err; do file="$run_dir/\${job_name}_\${job}.\${suffix}"; if test -f "$file"; then printf '\\nFILE=%s\\n' "$file"; tail -c 30000 "$file"; fi; done; fi; printf '\\nSLURM_STATE=%s\\nSLURM_EXIT_CODE=%s\\nSLURM_ELAPSED=%s\\n' "$state" "$exit_code" "$elapsed"`;
  const result = await runRemote("HPC", strictRemote("HPC", body), exec, 60000);
  const stdout = String(result.stdout ?? "");
  const stateValue = /^SLURM_STATE=(\S+)/mu.exec(stdout)?.[1]?.replace(/\+$/u, "") ?? null;
  const exitCodeValue = /^SLURM_EXIT_CODE=(\S*)/mu.exec(stdout)?.[1] || null;
  const elapsedValue = /^SLURM_ELAPSED=(\S*)/mu.exec(stdout)?.[1] || null;
  const workloadOutputSeen = /^FILE=/mu.test(stdout) && /DSH_SLURM_START=1/u.test(stdout) && /DSH_SLURM_DONE=1/u.test(stdout);
  const scheduler = { state: stateValue, exitCode: exitCodeValue, elapsed: elapsedValue, workloadOutputSeen };
  if (result.exitCode === 0 && stateValue) {
    updateWorkloadFromScheduler(findWorkloadRun(state, manifest.project, jobId), scheduler);
    if (TERMINAL_SCHEDULER_STATES.has(stateValue)) markAllocationTerminal(state, jobId, stateValue);
  }
...
async function fetchArtifacts({ manifest, requested, exec, userQuestions, shell, runRemote, config }) {
  ...
  const discovery = await runRemoteWithRetry(runRemote, "HPC", strictRemote("HPC", `set -eu; cd -- ${root}; for f in ${forList}; do if test -f "$f"; then printf 'OK|%s|%s|%s\\n' "$f" "$(stat -c %s "$f")" "$(sha256sum "$f" | cut -d' ' -f1)"; else printf 'MISSING|%s\\n' "$f"; fi; done`), exec, 30000);
  ...
  for (const rel of rels) {
    ...
    try {
      ...
      const result = await runLocal(shell, rcloneCopyfromCommand(rcloneRemote, `${manifest.remoteRoot}/${rel}`, tmpPath), 120000, exec.signal);
      ...
      try { await rm(tmpPath, { force: true }); } catch { /* never created or already removed */ }
      errors.push(`${rel}: ${String(error?.message ?? error)}`);
    }
  }
  ...
  await writeFile(receiptPath, JSON.stringify({ schema_version: 1, project: manifest.project, fetched_at: new Date().toISOString(), source_root: manifest.remoteRoot, rclone_remote: rcloneRemote, transfer: "rclone-copyto", files: transferred }, null, 2) + "\n", "utf8");
  const summary = transferred.map((item) => `OK ${item.rel} ${item.size} ${item.sha256}`).join("\n");
  return { stdout: `${summary}\nPINNED_FETCH_OK ${receiptPath}\n`, stderr: "", bytes: totalBytes, receiptPath };
}

// Run records carry the operation's FULL pinned resources (audit 2026-08-25
// P2-F3): genbio_pinned_job records the manifest job spec's cpus/gpus and
...
function startTrackedJob({ jobs, exec, state, project, operation, resources, runBody, label, logMaxBytes }) {
  ...
  run.jobId = jobs.start({ kind: "genbio-HPC-pinned", label, owner: exec.agent, run: () => {
    ...
    const done = (async () => {
      try {
        ...
        return { status: run.status, detail: run.error ?? "pinned operation failed" };
      } finally { run.finishedAt = Date.now(); }
    })();
    return { cancel: (reason) => controller.abort(reason ?? "pinned operation cancelled"), done, readOutput: () => { const text = [run.stdout, run.stderr && `[stderr]\n${run.stderr}`, run.error && `[error] ${run.error}`].filter(Boolean).join("\n"); run.stdout = ""; run.stderr = ""; return text; } };
  } });
  state.runs.push(run);
...
export function createPinnedTools({ makeTool, requirePolicy, requireState, publicState, runRemote, shell, userQuestions, jobs, config, requireRemoteAccess }) {
  ...
  const stageTool = makeTool("genbio_pinned_stage", "Stage one pinned project's fixed file set (owner-approved transfer) into its fixed HPC run directory, build PREPARED_SHA256.txt from the transferred bytes, and verify checksums plus a bounded fixed clean-environment bash -n on shell/sbatch files. No manifest-selected interpreter (python_bin) is ever executed remotely; no allocation is submitted. Projects are user-owned YAML manifests in the pinned-projects directory; unknown or malformed projects fail closed with the list of available projects.", { project: { type: "string", required: true } }, async (args, exec) => {
    const state = requireState(exec); requirePolicy();
    const manifest = await loadManifest(projectsDir, String(args.project));
    // Staging is a transfer/integrity helper, not a Slurm allocation. It still
    // requires a valid policy, explicit material-transfer approval, remote-root
    // authorization, checksum verification, and bounded clean-env syntax
    // checks, but it does not require or consume a compute resource envelope.
    await requireRemoteAccess("HPC", [{ root: manifest.remoteRoot, write: true }], exec, state);
    const run = startTrackedJob({ jobs, exec, state, project: manifest.project, operation: "stage", resources: { cpus: 0, gpus: 0, concurrency: 1 }, label: `HPC pinned ${manifest.project} stage`, logMaxBytes: config.logMaxBytes, runBody: async ({ exec: runExec }) => {
      const outcome = await stageAndValidate({ manifest, exec: runExec, userQuestions, shell, runRemote, config });
      return { stdout: outcome.validation.stdout, stderr: outcome.validation.stderr, exitCode: 0 };
    } });
    return { ok: true, status: { ...publicState(state), started: run } };
  });

  const jobTool = makeTool("genbio_pinned_job", "Submit one pinned Slurm job for a project (from its manifest's jobs map) after re-verifying the staged package checksums on both sides. Returns the Slurm job id immediately; collect terminal evidence with genbio_pinned_status. Exact-once: an ambiguous submission (transport loss or malformed job-id output) is reconciled read-only and never resubmitted; accepted jobs reserve envelope capacity until terminal evidence. Projects are user-owned YAML manifests in the pinned-projects directory; unknown projects/operations fail closed.", { project: { type: "string", required: true }, operation: { type: "string", required: true } }, async (args, exec) => {
    const state = requireState(exec); const policy = requirePolicy(); if (!policy.targets.HPC?.allowlist) throw new Error("active policy missing HPC allowlist");
    const project = String(args.project); const operation = String(args.operation);
    const manifest = await loadManifest(projectsDir, project);
    if (!SAFE_NAME_RE.test(operation) || !Object.hasOwn(manifest.jobs, operation)) throw new Error(`unknown operation for project ${manifest.project}: ${operation} (available: ${Object.keys(manifest.jobs).join(", ")})`);
    const jobSpec = manifest.jobs[operation];
    validateEnvelope(state, { cpus: jobSpec.cpus, gpus: jobSpec.gpus, concurrency: jobSpec.concurrency });
    // Aggregate envelope capacity (t7 / t5#3) is enforced authoritatively in
    // the atomic admission block below — still local and before any remote
    // action, but in the same synchronous block as the reservation so the
    // check sees every in-flight "submitting" reservation.
    // Phase-0 local validation of the EXACT template bytes (t3 matrix). The
    // returned digest binds those validated bytes to the pre-submit re-read
    // (t5#4): mutated bytes can never reach sbatch.
    const { templateSha } = await validateLocalJobTemplate(manifest, operation, policy, state);
    // ── Atomic in-flight admission reservation (t10 follow-up 1) ──
    // Contiguous, no await between the check and the pushes: atomic under the
    // single-threaded event loop, so two CONCURRENT executes for the same
    // (project, operation) can never both reserve — the second is rejected
    // before any sbatch (the concurrent double-submit race is closed). The
    // reservation also reserves full resources as "submitting" immediately,
    // so the aggregate capacity check stays accurate for in-flight work.
    const inFlight = allocationsOf(state).some((entry) => entry.project === manifest.project && entry.operation === operation && entry.status === "submitting");
    if (inFlight) throw new Error(`${manifest.project}: operation ${operation} already has an in-flight submission; wait for it to settle (or collect terminal evidence with genbio_pinned_status) before submitting again`);
    // Authoritative aggregate capacity gate (t7 / t5#3), inside the atomic
    // block: it sees every in-flight "submitting" reservation already pushed,
    // so concurrent admissions — even for DIFFERENT operations — cannot
    // jointly exceed the envelope.
    ...
    const intent = { project: manifest.project, operation, templateSha, token, uniqueJobName: null, status: "attempted", slurmJobId: null, submittedAt: Date.now() };
    const allocation = { slurmJobId: null, project: manifest.project, operation, cpus: jobSpec.cpus, gpus: jobSpec.gpus, concurrency: jobSpec.concurrency, status: "submitting", submittedAt: Date.now(), source: "admission" };
    ...
    try {
      ...
      // Admission did not proceed to submission: release the reservation so
      // the pair is not locked.
    ...
    const run = startTrackedJob({ jobs, exec, state, project: manifest.project, operation, resources: { cpus: jobSpec.cpus, gpus: jobSpec.gpus, concurrency: jobSpec.concurrency }, label: `HPC pinned ${manifest.project} ${operation}`, logMaxBytes: config.logMaxBytes, runBody: async ({ exec: runExec }) => {
      try {
        const outcome = await submitJob({ manifest, operation, policy, state, exec: runExec, runRemote, intent, allocation, templateSha });
        return { stdout: `${outcome.stdout}\n`, stderr: outcome.stderr, exitCode: 0, slurmJobId: outcome.slurmJobId };
      } catch (error) {
        // Settle the in-flight reservation only if it is still "submitting"
        // (submitJob threw before any of its own transitions); non-"submitting"
        // states (submitted/ambiguous/failed) are kept as-is.
        // sbatchIssued distinguishes the two safety outcomes:
        //   set   → the sbatch dispatch was in flight when the exception hit
        //           (the job MAY have been accepted): the attempt ends
        //           AMBIGUOUS and the exact-once gate keeps the pair from
        //           re-entering (read-only reconciliation only) until resolved.
        //   unset → no sbatch was dispatched: a DEFINITE failure, safe to
        //           release so the pair can be retried.
  ...
  const statusTool = makeTool("genbio_pinned_status", "Collect bounded read-only evidence for one Slurm job id of a pinned project: sacct accounting (State/ExitCode/Elapsed) plus the tail of the job's output and error files under the project run directory. No allocation, no writes.", { project: { type: "string", required: true }, job_id: { type: "string", required: true } }, async (args, exec) => {
    const state = requireState(exec); requirePolicy();
    const manifest = await loadManifest(projectsDir, String(args.project));
    await requireRemoteAccess("HPC", [{ root: manifest.remoteRoot, write: false }], exec, state);
    const jobId = String(args.job_id);
    const run = startTrackedJob({ jobs, exec, state, project: manifest.project, operation: `status-${jobId}`, resources: { cpus: 0, gpus: 0, concurrency: 1 }, label: `HPC pinned ${manifest.project} status ${jobId}`, logMaxBytes: config.logMaxBytes, runBody: async ({ exec: runExec }) => {
      const result = await statusJob({ manifest, jobId, state, exec: runExec, runRemote });
  ...
  const cancelTool = makeTool(
    "genbio_pinned_cancel",
    "Cancel one session-owned pinned Slurm job after explicit confirmation; broad user-wide or partition-wide cancellation is not supported.",
    { project: { type: "string", required: true }, job_id: { type: "string", required: true } },
    async (args, exec) => {
      const state = requireState(exec); requirePolicy();
      const jobId = String(args.job_id);
      if (!/^[0-9]{1,10}$/u.test(jobId)) throw new Error(`invalid Slurm job id: ${jobId}`);
      const owned = (state.allocations ?? []).find((entry) => entry.project === String(args.project) && entry.slurmJobId === jobId && ["submitting", "nonterminal", "ambiguous"].includes(entry.status));
      if (!owned) throw new Error(`Slurm job ${jobId} is not an active session-owned job for project ${args.project}`);
      if (!userQuestions) throw new Error("cancellation requires the DSH user-question provider");
      const questionId = `genbio-pinned-cancel-${jobId}`;
      const answer = await userQuestions.ask({ agent: exec.agent, signal: exec.signal, questions: [{ id: questionId, header: "Cancel Slurm job", question: `Cancel session-owned Slurm job ${jobId} for project ${args.project}?`, options: [{ label: "Cancel this job", description: "Issue exactly one cancellation request for this job ID." }, { label: "Keep running", description: "Do not cancel the job." }] }] });
      ...
      const body = `set -eu; scancel ${shellQuote(jobId)}`;
      const result = await runRemote("HPC", strictRemote("HPC", body), exec, 30000);
      if (result.exitCode !== 0) throw new Error(`scancel failed: ${result.stderr || result.stdout || result.exitCode}`);
      owned.status = "cancel-requested"; owned.source = "explicit scancel request";
      return { ok: true, status: { ...publicState(state), cancellation: { jobId, requested: true, terminalEvidence: "pending status refresh" } } };
    },
  );

  const fetchTool = makeTool("genbio_pinned_fetch", "Retrieve one pinned project's allowlisted small report/artifact files (manifest fetch.files, byte-capped by fetch.max_bytes) from its fixed HPC run directory into the manifest's local fetch destination. rclone-only download; the remote SHA-256 of every file must equal the local SHA-256 after transfer, and a dated FETCH_RECEIPT json is written next to the files. Read-only on the remote run dir; no allocation and no envelope is required. Omit files to retrieve every allowlisted file.", { project: { type: "string", required: true }, files: { type: "array", items: { type: "string" } } }, async (args, exec) => {
    const state = requireState(exec); requirePolicy();
    const manifest = await loadManifest(projectsDir, String(args.project));
    if (!manifest.fetch) throw new Error(`${manifest.project}: manifest has no fetch section; add fetch: { max_bytes, dest, files } to enable bounded retrieval`);
    await requireRemoteAccess("HPC", [{ root: manifest.remoteRoot, write: false }], exec, state);
    const requested = Array.isArray(args.files) ? args.files.map((value) => String(value)) : [];
    const run = startTrackedJob({ jobs, exec, state, project: manifest.project, operation: "fetch", resources: { cpus: 0, gpus: 0, concurrency: 1 }, label: `HPC pinned ${manifest.project} fetch`, logMaxBytes: config.logMaxBytes, runBody: async ({ exec: runExec }) => {
      const outcome = await fetchArtifacts({ manifest, requested, exec: runExec, userQuestions, shell, runRemote, config });
      return { stdout: outcome.stdout, stderr: outcome.stderr, exitCode: 0 };
    } });
    return { ok: true, status: { ...publicState(state), started: run } };
  });

  return { stageTool, jobTool, statusTool, cancelTool, fetchTool, projectsDir };
}

// Additive export surface for the declarative recipe executor (lib/project-tools.js)
// and future generalizations. Purely additive — the existing pinned v1 path and its
// behavior are unchanged. probeGpu03Headroom is exported via its own `export async
// function` declaration above; strictRemote is re-listed for one-stop importability.
export {
  DEFAULT_PROJECTS_DIR, parseManifest, strictRemote,
  validateEnvelope, assertAggregateCapacity, allocationsOf, submissionsOf, uniqueJobNameFor,
  verifyPackage, localFiles, runLocal, parseChecksums, parseSubmitOutput, isTransportFailure,
  reconcileSubmission, startTrackedJob, askTransfer, submitJob, stageAndValidate, stageRecipeWrapper,
  statusJob, findOwnedOperationRun, updateWorkloadFromScheduler, TERMINAL_SCHEDULER_STATES,
};
