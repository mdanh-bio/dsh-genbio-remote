...
function assertAbsolutePath(value, label) {
  if (typeof value !== "string" || !value.startsWith("/") || value.includes("..") || value.includes("\0") || /[\r\n]/u.test(value)) throw new Error(`${label} must be an absolute path without .., NUL, or newlines`);
}
...
export function parseProjectManifest(project, parsed) {
  ...
  if (version !== 1 && version !== 2) throw new Error(`${project}: schema_version must be 1 or 2`);
  const topKeys = new Set(["schema_version", "project", "description", "local_root", "remote_root", "python_bin", "files", "extra_dirs", "jobs", "fetch"]);
  ...
  if (version === 2 && parsed.python_bin !== undefined) throw new Error(`${project}: python_bin is not permitted in schema_version 2`);
  if (version === 1 && parsed.python_bin !== undefined) assertAbsolutePath(parsed.python_bin, `${project}: python_bin`);
  ...
  for (const [name, spec] of Object.entries(parsed.jobs)) {
    ...
    if (version === 1) {
      assertKeys(spec, new Set(["template", "cpus", "gpus", "concurrency", "node"]), `${project}: job ${name}`);
      assertProjectRelativePath(spec.template, `${project}: job ${name} template`);
      jobs[name] = Object.freeze({ template: spec.template, ...resources });
    } else {
      assertKeys(spec, new Set(["template", "recipe", "cpus", "gpus", "concurrency", "node"]), `${project}: job ${name}`);
      const forms = Number(spec.template !== undefined) + Number(spec.recipe !== undefined);
      if (forms !== 1) throw new Error(`${project}: job ${name} must define exactly one of template or recipe`);
      if (spec.template !== undefined) {
        assertProjectRelativePath(spec.template, `${project}: job ${name} template`);
        jobs[name] = Object.freeze({ template: spec.template, ...resources });
      } else jobs[name] = Object.freeze({ recipe: parseRecipe(project, name, spec.recipe, files), ...resources });
    }
  }
  return Object.freeze({ schemaVersion: version, project: parsed.project, localRoot: parsed.local_root, remoteRoot: parsed.remote_root, pythonBin: version === 1 ? parsed.python_bin ?? null : null, files, extraDirs, jobs: Object.freeze(jobs), fetch: parseFetch(project, parsed.fetch), description: typeof parsed.description === "string" ? parsed.description : "" });
}
...
export function buildOperationPlan({ project, operation, policyHash, manifestSha, resolution }) {
  if (!/^[a-f0-9]{64}$/u.test(policyHash ?? "") || !/^[a-f0-9]{64}$/u.test(manifestSha ?? "")) throw new Error("plan requires full policy and manifest SHA-256 values");
  const plan = Object.freeze({ schema: "genbio-plan/1", project, operation, policyHash, manifestSha, bytesSha: resolution.bytesSha, jobName: resolution.jobName, cpus: resolution.cpus, gpus: resolution.gpus, concurrency: resolution.concurrency, node: resolution.node, partition: resolution.partition, script: resolution.script, parameters: resolution.parameters });
